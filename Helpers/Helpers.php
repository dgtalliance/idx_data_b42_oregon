...
class Helpers
{
    ...
    function getAllActivePendingProperties($status, $skip, $lastUpdate = NULL)
    {
        ...
        $url = 'https://resoapi.rmlsweb.com/reso/odata/Property?$expand=Media($select=MediaURL,Order,PrivateYn)&$select=' . $select . '&$skip=' . $skip . '&$filter=' . $filter . '&$top=100';
    ...
    )
    {
        ...
        if ($property['InternetAddressDisplayYn'] == 0) {
            $st_name = "Undisclosed Address";
            $address_short = "Undisclosed Address";
        }
